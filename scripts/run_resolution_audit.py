...
def generate_markdown_report(
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        f.write("### 測試環境矩陣\n\n")
        f.write("| 序號 | 模擬環境名稱 | 寬度 (Width) | 高度 (Height) | DPI 縮放比例 | 適用物理螢幕示例 |\n")
        f.write("| :---: | :--- | :---: | :---: | :---: | :--- |\n")
        ...
        f.write("## 2. 使用環境適用性與風險分析 (環境維度)\n\n")
        
        # 27吋
        f.write("### 2.1 27吋專業螢幕使用環境 ($2560 \\times 1440$ @1.5x)\n")
        f.write("- **風險級別**：`LOW`\n")
        f.write("- **適用性評估**：高解析度搭配 1.5x DPI 縮放。主視窗可見工作區寬裕。經幾何稽核，Matplotlib 圖表以及所有控制面板皆能完美展開，字體與圖例顯示清晰，非常適合此系統的日常高密度製程診斷工作。\n\n")
        
        # 32吋
        f.write("### 2.2 32吋 4K 旗艦螢幕使用環境 ($3840 \\times 2160$ @2.0x)\n")
        f.write("- **風險級別**：`LOW`\n")
        f.write("- **適用性評估**：超高解析度搭配 2.0x DPI 縮放。在 2.0x 大字型與大元件模式下，系統能正確套用 Windows DPI 自適應縮放，版面依然緊湊且無元件重疊。Matplotlib 的字級與圖表圖例保持同比例適應，呈現頂級的視覺質感。\n\n")

        # 筆電
        f.write("### 2.3 筆電螢幕使用環境 ($1366 \\times 768$ @1.0x / $1920 \\times 1080$ @1.25x)\n")
        f.write("- **風險級別**：`LOW` 至 `MEDIUM`\n")
        f.write("- **適用性評估**：\n")
        f.write("  - 在標準 1080p @1.25x 下佈局比例完美，元件易讀，無任何功能性溢出。\n")
        f.write("  - 在極端 1366x768 筆電低解析度下，系統的 `WINDOW_MIN_HEIGHT = 640` 發揮了保護作用。然而，側邊控制面板的「重新分析」與「下一步」按鈕此時已非常逼近底邊，若使用者搭配了較寬的 Windows 工作列，主視窗可能會被微幅擠壓，建議引導使用者收合側欄控制條件以保留最大工作區。\n\n")

        # 投影機
        f.write("### 2.4 投影機使用環境 ($1024 \\times 768$ @1.0x / $1280 \\times 720$ @1.0x)\n")
        f.write("- **風險級別**：`HIGH` (關鍵風險區域)\n")
        f.write("- **適用性評估**：\n")
        f.write("  - 在投影機常用的 1024x768 (4:3) 解析度下，由於高度僅有 768px，且寬度僅有 1024px，側欄與主工作區皆被高度壓縮。\n")
        f.write("  - **關鍵風險**：資料設定頁 (Data Setup) 在空狀態時可勉強顯示，但在載入資料後，高密度的工單表格、量測檔案列表與 Readiness Bar (Start Analysis 按鈕) 在 768px 高度下將會溢出 (Overflow) 出主視窗邊界之外！使用者將「完全無法看到或點擊 Start Analysis 按鈕」，這是一個 Block 的嚴重風險。\n")
        f.write("  - 此外，投影機流明度低，介面使用的「Slate 淺灰色」底色在強光或低流明投影機上對比度可能稍嫌不足，可能會降低現場操作人員的可讀性。\n\n")

        f.write("## 3. 幾何稽核發現之缺陷與風險清單 (Defect List)\n\n")
        if total_cnt == 0:
            f.write("*恭喜！無發現任何幾何佈局或文字裁切異常。*\n")
        else:
            f.write("| ID | 模擬環境 | 頁面 | 異常類型 | 異常組件 / Widget | 幾何與現象說明 | 嚴重度 |\n")
            f.write("| :---: | :--- | :--- | :---: | :--- | :--- | :---: |\n")
            ...
            f.write("## 4. 具體修復與改進建議\n\n")
            f.write("1. **為極低解析度 (如 1024x768) 引進 QScrollArea 包裹**：\n")
            f.write("   - **問題**：資料設定頁 (`DataSetupPage`) 與工單頁在 768px 以下高度時，底部主動作按鈕 (Start Analysis / 儲存) 會因為溢出而不可見且無法點選。\n")
            f.write("   - **建議**：在 `DataSetupPage` 的主佈局中，引入 `QScrollArea` 將整個內容包覆。當視窗高度小於 800px 時，自動出現垂直捲軸，確保使用者可以向下捲動點擊所有動作按鈕。\n\n")
            f.write("2. **側欄控制條件自動收合門檻調整**：\n")
            f.write("   - **問題**：在高度小於 720px 時，側欄佈局過於擁擠，重新分析按鈕與篩選欄位可能發生重疊。\n")
            f.write("   - **建議**：調校 `SIDEBAR_CONDITIONS_COLLAPSE_HEIGHT` (目前為 720px)。當視窗高度小於 768px 時，強制將側邊篩選條件收合，僅保留流程選單與底部主動作按鈕，騰出空間給核心按鈕。\n\n")
            f.write("3. **投影機專用高對比/暗色主題預留**：\n")
            f.write("   - **問題**： Slate 淺灰色底色搭配 Electric Blue 對比，在投影機低對比度與高環境光下，表格格線與 MUTED 灰字 (如單位、副標題) 容易褪色模糊。\n")
            f.write("   - **建議**：在主題設定中，為投影機環境提供一鍵切換「高對比暗色主題」的選項，或調深 `TEXT_MUTED` 的顏色至 `#334155`，確保投影畫面可讀。\n")
