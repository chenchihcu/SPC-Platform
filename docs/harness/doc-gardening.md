...
Doc gardening keeps the SPC/SPI repo knowledge base useful for agent work without adding a second governance system.
...
- Cursor noise control: `agents_gateway.mdc` remains always-on and `vibe-tools.mdc` remains optional, not always-on.
...
Active residual risk still belongs in `docs/open-questions.md`.
