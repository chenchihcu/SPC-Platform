...
- **DB-backed semantic validation for chart statistics**: Payload contract checks, `metadata.is_valid`, and renderability checks are not sufficient proof of statistical correctness. Any change to analytics engines, `chart_registry.py`, payload slicing, density semantics, pair expansion, triple-feature semantics, or report chart resolution must validate against real DB/session data when available by running `scripts/validate_db_chart_semantics.py`. Expected values must be independently recomputed from the joined measurement+coordinate dataframe and active spec.
...
- **Filter Infinite Values Safely**: Always explicitly sanitize `np.inf` and `-np.inf` (using `.replace([np.inf, -np.inf], np.nan)`) before `.dropna()` in SPC engines and DataFrame aggregations to prevent infinite bounds propagation to computations and Matplotlib axes. Guard checks alone are not sufficient; the compute path must sanitize immediately before aggregation.
