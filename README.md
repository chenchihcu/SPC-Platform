...
- **② 分析 (Analysis)**：
    - **統計圖表 (Chart Analysis)**：支援 1F/2F/3F 多特徵同步分析（Volume/Area/Height）；側欄「分析特徵」變更會重新計算，圖表頁「圖表檢視組合」則在已預算的單／雙／三變量組合間即時切換而不重算。所有 Matplotlib 圖表共用 `BaseChart` 視覺語意（量測線、中心線、管制限、規格限、OOC/OOS 標記、樣本揭露），圖卡具備 `Ready/Incompatible/NoData/Error` 狀態標籤；純文字摘要（失控、偏移、漂移、離群）改由同列左側選單 `統計資料` 以一頁式資料表瀏覽。擴充圖表：Hotelling T² 多變量管制圖、Radar 綜合比較圖、Moran's I LISA 空間自相關圖。
...
- **③ 輸出 (Output)**：
    - **專業報告匯出 (Report Export)**：一鍵產生 `engineering` 模板 PPTX 工程報告，支援圖表預覽、匯出範圍摘要、自動生成證據頁 (2x2 Chart Gallery) 與圖表證據覆蓋表；單特徵圖會依全部選定特徵展開、雙特徵配對圖會輸出所有配對，Boxplot/Pareto 另輸出可用的 Pad／Image 分組證據；報告會揭露資料來源、未納入證據與診斷證據類型，無有效 X/Y 座標時 Spatial Heatmap 與 LISA 不作為有效判讀證據。
